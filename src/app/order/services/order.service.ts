...
private apiUrl = `${environment.apiUrl}/order`;
  ...
  // ✅ Sahi (TypeScript style):
getAllOrdersByTenant(tenantId: number): Observable<ApiResponse<OrderResponseDto[]>> {
  return this.http.get<ApiResponse<OrderResponseDto[]>>(
    `${this.apiUrl}/tenant/${tenantId}/all`
  );
}
